...
public class ComplexSearchTest extends AbstractTest {
    ...
    public void noFiltersTest() {
        //when
        JsonPath response = complexSearch(new HashMap<>()).then().extract().jsonPath();

        //then
        assertTrue(response.getList("results").size() > 0);
        assertEquals(0, response.getInt("offset"));
        assertEquals(10, response.getInt("number"));
        assertTrue(response.getInt("totalResults") > 0);
    }
